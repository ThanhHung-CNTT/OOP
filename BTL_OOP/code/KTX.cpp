...
void Dormitory::timkiemAddress()
{
    ...
    while (i)
    {
        ...
        case 1:
        {
            ...
            cin.ignore();
            getline(cin, timkiem);
            char *diachi = new char[timkiem.length()];
            strcpy(diachi, timkiem.c_str());
            for(i=0;i<=strlen(diachi);i++){
                if(diachi[i]>=65&&diachi[i]<=90)
                    diachi[i]=diachi[i]+32;
            }
            for (int i = 0; i < list_person.size(); i++)
            {   
                char *kiemtra = new char[list_person[i]->getAddress().length()];
                strcpy(kiemtra, list_person[i]->getAddress().c_str());
                for(i=0;i<=strlen(kiemtra);i++){
                    if(kiemtra[i]>=65&&kiemtra[i]<=90)
                        kiemtra[i]=kiemtra[i]+32;
                }
                if(strcmp(kiemtra,diachi)==0){
                    if(list_person[i]->selection()==true){
                        inkhung();
                        cout<<"Da tim thay";
                    }
